...
#### Hacking
The source code that makes it into the library jar file is all in one `.cljs` file that only depends on cljs.pprint. However you can interact with the examples from a web page. The examples are all `defs` in ...  

##### See example devcards in a browser
boot dev
...
So far so good but at the moment it just crashes, so don't bother with the REPL!
